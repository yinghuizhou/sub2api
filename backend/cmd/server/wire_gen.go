...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	referralService := service.NewReferralService(referralRepository, userRepository, billingCacheService, client, settingService, configConfig)
	...
	adminService := service.NewAdminService(userRepository, groupRepository, accountRepository, soraAccountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, vendorRepository, proxyGroupService)
	...
	accountTestService := service.NewAccountTestService(accountRepository, vendorRepository, geminiTokenProvider, antigravityGatewayService, httpUpstream, configConfig)
	crsSyncService := service.NewCRSSyncService(accountRepository, proxyRepository, oAuthService, openAIOAuthService, geminiOAuthService, configConfig)
	sessionLimitCache := repository.ProvideSessionLimitCache(redisClient, configConfig)
	platformProxyRules := service.NewPlatformProxyRules()
	proxyAssignmentService := service.NewProxyAssignmentService(proxyRepository, platformProxyRules)
	accountHandler := admin.NewAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, compositeTokenCacheInvalidator, proxyAssignmentService, billingCacheService)
	...
	vendorProxyService := service.NewVendorProxyService(vendorRepository, accountRepository)
	vendorProbeService := service.NewVendorProbeService()
	...
	vendorHandler := admin.NewVendorHandler(vendorService, vendorProxyService, vendorProbeService, vendorHealthService, vendorBalanceService)
	...
	vpnEventService := service.NewVpnEventService(client)
	vpnAlertService := service.NewVpnAlertService(client, vpnEventService)
	vpnHandler := admin.NewVpnHandler(vpnAgentService, vpnEventService, vpnAlertService, adminService)
	...
	handlerPaymentHandler := handler.NewPaymentHandler(paymentService, settingService, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, soraGatewayHandler, handlerSettingHandler, totpHandler, idempotencyCoordinator, idempotencyCleanupService, handlerPaymentHandler)
	...
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, soraMediaCleanupService, schedulerSnapshotService, tokenRefreshService, accountExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, vendorBackgroundService, proxyHealthService)
