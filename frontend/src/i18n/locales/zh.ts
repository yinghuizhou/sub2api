export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      noProxy: '无代理',
      proxyGroup: '代理分组',
      proxyGroupPlaceholder: '输入分组名，如：美东共享组',
      proxyGroupHint: '共享代理组名称，优先级低于专用代理（proxy）',
      autoAssignProxy: '自动分配',
      autoAssignProxySuccess: '代理已自动分配：{group}',
      autoAssignProxyFailed: '自动分配失败',
      testProxyConnectivity: '测试连通性',
      testProxySuccess: '连通正常，延迟 {latency}ms，出口 IP：{ip}',
      testProxyFailed: '连通测试失败',
      concurrency: '并发数',
      priority: '优先级',
      ...
      quickMode: '快捷添加',
      standardMode: '标准模式',
      quickModeDesc: '粘贴 Session Key 快速添加',
      standardModeDesc: '完整配置所有选项',
      quickSessionKeyPlaceholder: '粘贴 Session Key，每行一个（支持批量添加）',
      quickNamePlaceholder: '留空自动生成（推荐）',
      quickSessionKeyHelp: '打开 claude.ai → F12 开发者工具 → Application → Cookies → 复制 __Secure-sessionKey 的值',
      quickSessionKeyRequired: '请粘贴至少一个 Session Key',
      quickAuthMethod: '授权方式',
      quickAuthOAuth: 'OAuth（完整权限）',
      quickAuthSetupToken: 'Setup Token（仅推理）',
      quickOptionalSettings: '可选设置',
    ...
    proxies: {
      ...
      healthCheckAll: '全部健康检查',
      healthCheckAllRunning: '正在执行健康检查...',
      healthCheckAllDone: '健康检查完成：{healthy} 正常，{unhealthy} 异常，{errors} 错误',
      healthCheckAllFailed: '健康检查失败',
      importManifest: '导入清单',
      importManifestTitle: '导入 Astrill 清单',
      importManifestHint: '选择 Astrill JSON 清单文件以批量导入 VPN 代理。',
      importManifestFailed: '清单导入失败',
